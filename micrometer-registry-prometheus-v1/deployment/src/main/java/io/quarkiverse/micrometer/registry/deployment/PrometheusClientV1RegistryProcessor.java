package io.quarkiverse.micrometer.registry.deployment;

import java.util.function.BooleanSupplier;

import org.jboss.logging.Logger;

import io.quarkiverse.micrometer.registry.prometheus.export.PrometheusRecorder;
import io.quarkiverse.micrometer.registry.prometheus.export.exemplars.EmptyExemplarSamplerProvider;
...
import io.quarkiverse.micrometer.registry.prometheus.export.exemplars.OpentelemetryExemplarSamplerProvider;
 ...
 * references.
 */
@BuildSteps(onlyIf = PrometheusClientV1RegistryProcessor.PrometheusEnabled.class)
public class PrometheusClientV1RegistryProcessor {
    private static final Logger log = Logger.getLogger(PrometheusClientV1RegistryProcessor.class);
    ...
    MicrometerRegistryProviderBuildItem createPrometheusRegistry(MicrometerConfig config,
        ...
        AdditionalBeanBuildItem.Builder builder = AdditionalBeanBuildItem.builder()
                .addBeanClass("io.quarkiverse.micrometer.registry.prometheus.export.PrometheusV1MeterRegistryProvider")
                .setUnremovable();
        if (config.export().prometheus().defaultRegistry()) {
            builder.addBeanClass("io.quarkiverse.micrometer.registry.prometheus.export.PrometheusV1MeterRegistryProducer");
        }
