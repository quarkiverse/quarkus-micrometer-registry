package io.quarkiverse.micrometer.registry.prometheus.deployment.deployment.binder;
...
import io.quarkiverse.micrometer.registry.prometheus.deployment.test.HelloResource;
import io.quarkiverse.micrometer.registry.prometheus.deployment.test.MeterResource;
...
public class HttpDevModeConfigTest {
    ...
    public void test() throws Exception {
        ...
        when().get("/test/requests").then().statusCode(200)
                .log().body()
        ...
        when().get("/test/requests").then().statusCode(200)
                .log().body()
                .body(not(containsString("/hello/{message}")))
                .body(not(containsString("/q/dev"))) // ignored by default
                .body(not(containsString("/q/metrics"))); // ignored by default

        test.modifyResourceFile("application.properties",
                s -> s.replace("orange=banana",
        ...
        when().get("/test/requests").then().statusCode(200)
                .log().body()
                .body(not(containsString("/hello/{message}")))
                .body(containsString("/q/dev"))
                .body(containsString("/q/metrics"));
        ...
        test.modifyResourceFile("application.properties",
                s -> s.replace("quarkus.http.non-application-root-path=/",
        ...
        test.modifyResourceFile("application.properties",
                s -> s.replace("quarkus.http.non-application-root-path=/bob",
        ...
        when().get("/bob/test/requests").then().statusCode(200)
                .log().body()
                .body(not(containsString("dev"))) // ignored
        ...
        when().get("/bob/test/requests").then().statusCode(200)
                .log().body()
                .body(not(containsString("/george/dev"))) // ignored, http root removed
